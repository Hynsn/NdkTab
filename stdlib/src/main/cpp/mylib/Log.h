...
void pipeSize(){
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, LOG_TGA, "pipesize1 %ld",length);
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, LOG_TGA, "pipesize2 %ld",length);
}
...
Java_com_ndk_std_Std_unbindLogPipe(JNIEnv* env, jobject /* this */) {
    if(pipRet == 0){
        pipRet = -1;
        close(log_fd[0]);
        close(log_fd[1]);
...
Java_com_ndk_std_Std_bindLogPipe(
...
Java_com_ndk_std_Std_readMessage(JNIEnv* env, jobject thiz){
    if(pipRet==0){
        fcntl(log_fd[0], F_SETFL, O_NONBLOCK);
        ssize_t ret = read(log_fd[0], log_buff, sizeof(log_buff));
...
extern "C" JNIEXPORT jint JNICALL
Java_com_ndk_std_Std_writeMessage(JNIEnv* env, jobject thiz, jstring message){
    ssize_t ret = -1;
    if(pipRet==0) {
        const char *buff = env->GetStringUTFChars(message, 0);
        ret = write(log_fd[1], buff, strlen(buff));
        __android_log_print(ANDROID_LOG_VERBOSE, LOG_TGA, "write data %zu",strlen(buff));
        env->ReleaseStringUTFChars(message,buff);
    }
    return static_cast<jint>(ret);
}
extern "C" JNIEXPORT jint JNICALL
Java_com_ndk_std_Std_testPipeSize(JNIEnv* env, jobject thiz){
